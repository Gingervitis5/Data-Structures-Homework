public class MyLinkedList {
	...
	private MyLinkedList reverse(ListNode node) {
		if (node == null) {
			...
			MyLinkedList nl = reverse(node.next); 
			nl.addLast(node.data);
			return nl;
		}
	...
	private ListNode reverse(ListNode prev, ListNode subHead) {
		...
		if (subHead == null) {
			...
			h = reverse(subHead, subHead.next); //This will reverse the sublist
			subHead.next = prev; //Reverses the list
			prev.next = null; //Set the first node.next to null since its in the last position
		}
